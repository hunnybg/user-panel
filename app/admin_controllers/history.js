...
exports.admin_history = function (request_data, response_data) {
    utils.check_request_params(request_data.body, [{name: 'start_date'}, {name: 'end_date'}], function (response) {
        if (response.success) {
            ...
            if (page) {
                Order.aggregate([condition1, created_by_condition, order_type_condition, order_status_id_condition, user_query, store_query, order_payment_query, cart_query, array_to_json_cart_query, request_query, review_query, array_to_json_user_detail, array_to_json_store_detail, array_to_json_order_payment_query, array_to_json_request_query, provider_query, array_to_json_provider_query, payment_gateway_query, pickup_type_condition, payment_status_condition, search, filter, count]).then((orders) => {
                    if (orders.length === 0) {
                        response_data.json({
                            success: false,
